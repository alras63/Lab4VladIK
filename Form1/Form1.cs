...
namespace Form1
{
    public partial class Form1 : Form
    {
        ...
        float bX = 10, bY = 6; //вторая точка

        //две линии вне прямоугольник
        float out1X1, out1Y1, out1X2, out1Y2; //первая линия - координаты начала и конца
        float out2X1, out2Y1, out2X2, out2Y2; //вторая линия - координаты начала и конца
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if(isResult)
            {
                float x1StartLine = x0 - ((out1X1) * (-dx));
                float y1StartLine = y0 + ((out1Y1) * (-dy));

                float x2StartLine = x0 - ((out2X1) * (-dx));
                float y2StartLine = y0 + ((out2Y1) * (-dy));


                e.Graphics.DrawLine(Pens.Green, x1StartLine, y1StartLine, x0 + out1X2 * dx, y0 - out1Y2 * dy);
                e.Graphics.DrawLine(Pens.Green, x2StartLine, y2StartLine, x0 + out2X2 * dx, y0 - out2Y2 * dy);
            }
